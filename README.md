...
* The first chart (top left) shows the types of cards used for transactions (x-axis) in relation to the number of transactions made (y-axis). At the same time, I added a legend listing all the cities and sorted them in ascending order by the number of cities. The difference in Tehran’s figures is clearly evident, both in terms of payment cards and the number of cities using them.
* The second figure (bottom left) shows the time at which the transactions took place (x-axis) in relation to the number of cities where they occurred. As before, I added a legend so that the names of the cities are also visible in the figure. Tehran is once again in first place by a wide margin compared to the other cities, particularly at 11 a.m., when the most transactions took place compared to all other times of the day.
* Finally, the third graph (top right) shows the volume of transactions (x-axis) in relation to the number of successful and unsuccessful transactions. I added a legend to show their status, and almost all transactions have more failed attempts than successful ones. 
